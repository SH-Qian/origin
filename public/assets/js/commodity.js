...
$(function () {
	...
	$(".rdPro dl").hover(function () {
		...
		$(this).removeClass("rddlstyle"); $(this).stop(true, true).animate({
			opacity: "1"
		})
	})
	//.upd,.add
	...
	//提交到购物车,存入cart表
	$("#cart").click(function () {
		var cart = $(".posCur").attr('name');

		if (cart=='add') {
			var comid = $(".rongliang").attr('name');
			var number = $(".number").val();

			$.post("/ajax_commodity_cart",
			{
				cid:comid,
				num:number,
				art:cart
			...
			function(data,status){
				...
				if (data.code>0) {
					var hostname = 'http://'+$(location).attr('hostname')+'/commodity/cart';
			...
			// 获取name为checkbox_name的checkbox元素
			...
			function(data,status){
				...
				if (status=='success'&&data.code>0) {
					var hostname = 'http://'+$(location).attr('hostname')+'/commodity/cart/submit';
					$(location).attr('href',hostname);
				}else{
					alert(data.msg);
				}
	...
	//取消支付,修改订单
	$("#qxzf").click(function () {
		var type = $(".posCur").attr('name');
		var ordid = $(".qxzf").attr('name');
